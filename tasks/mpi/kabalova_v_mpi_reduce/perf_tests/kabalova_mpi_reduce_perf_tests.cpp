...
TEST(kabalova_v_mpi_reduce, test_pipeline_run) {
  ...
  kabalova_v_mpi_reduce::createVector(vec);
  ...
  // Create Task
  auto testMpiTaskParallel = std::make_shared<kabalova_v_mpi_reduce::TestMPITaskParallel>(taskDataMpi, "+");
...
TEST(kabalova_v_mpi_reduce, test_task_run) {
  boost::mpi::communicator world;
  size_t vecSize = 200000;
  std::vector<int> vec(vecSize);
  kabalova_v_mpi_reduce::createVector(vec);
