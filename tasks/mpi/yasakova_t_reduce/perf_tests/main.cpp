...
TEST(yasakova_t_reduce, test_pipeline_run) {
  std::random_device dev;
  std::mt19937 gen(dev());
  ...
  if (world.rank() == 0) {
    ...
    global_matrix = getRandomMatrix(rows, columns, upper_border, lower_border);

    int rnd_rows = gen() % rows;
    int rnd_cols = gen() % columns;
  ...
  auto testMpiTaskParallel = std::make_shared<yasakova_t_reduce::TestMPITaskParallel>(taskDataPar);
...
TEST(yasakova_t_reduce, test_task_run) {
  std::random_device dev;
  std::mt19937 gen(dev());
  boost::mpi::communicator world;
  std::vector<std::vector<int>> global_matrix;
