...
TEST(mpi_muradov_m_matrix_multiply_ccs, test_pipeline_run) {
  ...
  if (world.rank() == 0) {
    A_ = muradov_m_matrix_multiply_ccs_mpi::gen_rand_matrix(size, size, elements);
    B_ = muradov_m_matrix_multiply_ccs_mpi::gen_rand_matrix(size, size, elements);
  }
  ...
  if (world.rank() == 0) {
    auto exp_C = muradov_m_matrix_multiply_ccs_mpi::multiply_matrices(A_, B_);
    muradov_m_matrix_multiply_ccs_mpi::convert_to_CCS(exp_C, exp_C.size(), exp_C[0].size(), exp_C_val, exp_C_row_ind,
                                                      exp_C_col_ptr);
  }
  ...
  muradov_m_matrix_multiply_ccs_mpi::convert_to_CCS(A_, rows_A, cols_A, A_val, A_row_ind, A_col_ptr);
  muradov_m_matrix_multiply_ccs_mpi::convert_to_CCS(B_, rows_B, cols_B, B_val, B_row_ind, B_col_ptr);
  ...
  auto task = std::make_shared<muradov_m_matrix_multiply_ccs_mpi::MatrixMultiplyCCS>(task_data);
...
TEST(mpi_muradov_m_matrix_multiply_ccs, test_task_run) {
  boost::mpi::communicator world;
  int size = 256;
  int elements = 6553;
  std::vector<std::vector<double>> A_;
