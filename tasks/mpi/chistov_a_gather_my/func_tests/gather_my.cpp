...
TEST(chistov_a_gather_my, test_empty_vector) {
  ...
  if (world.rank() == 0) {
    ...
    local_vector = chistov_a_gather_my_test::getRandomVector<int>(0);
...
TEST(chistov_a_gather_my, test_incorrect_root_process) {
  ...
  if (world.rank() == 0) {
    std::vector<int> local_vector;
    std::vector<int> gathered_data;
    local_vector = chistov_a_gather_my_test::getRandomVector<int>(1);
...
TEST(chistov_a_gather_my, test_int_gather) {
  boost::mpi::communicator world;
  int count = 2;
  std::vector<int> local_vector = chistov_a_gather_my_test::getRandomVector<int>(count);
...
TEST(chistov_a_gather_my, test_double_gather) {
  boost::mpi::communicator world;
  int count = 2;
  std::vector<double> local_vector = chistov_a_gather_my_test::getRandomVector<double>(count);
...
TEST(chistov_a_gather_my, test_float_gather) {
  boost::mpi::communicator world;
  int count = 2;
  std::vector<float> local_vector = chistov_a_gather_my_test::getRandomVector<float>(count);
...
TEST(chistov_a_gather_my, test_char_gather) {
  boost::mpi::communicator world;
  int count = 2;
  std::vector<char> local_vector = chistov_a_gather_my_test::getRandomVector<char>(count);
