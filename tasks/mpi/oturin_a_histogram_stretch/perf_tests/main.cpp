...
TEST(oturin_a_histogram_stretch_mpi_perftest, test_pipeline_run) {
  uint32_t width = 2000;
  ...
  in[0] = 200;  // if maximum == minimum code skips most tough part
  in[3] = 0;
  ...
  // Create Task
  auto testMpiTaskParallel = std::make_shared<oturin_a_histogram_stretch_mpi::TestMPITaskParallel>(taskDataPar);
...
TEST(oturin_a_histogram_stretch_mpi_perftest, test_task_run) {
  uint32_t width = 2000;
  uint32_t height = 2000;
