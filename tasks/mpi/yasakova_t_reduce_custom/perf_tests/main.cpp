...
TEST(yasakova_t_reduce_custom_mpi, test_pipeline_run) {
  std::random_device dev;
  std::mt19937 gen(dev());
  ...
  auto testMpiTaskParallel = std::make_shared<yasakova_t_reduce_custom_mpi::TestMPITaskParallel>(taskDataPar);
...
TEST(yasakova_t_reduce_custom_mpi, test_task_run) {
  std::random_device dev;
  std::mt19937 gen(dev());
  boost::mpi::communicator world;
  std::vector<std::vector<int>> global_matrix;
