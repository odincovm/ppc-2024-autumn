...
TEST(sarafanov_m_quick_sort_batcher_merge_mpi, pipeline_run) {
  ...
  auto taskParallel = std::make_shared<sarafanov_m_quick_sort_batcher_merge_mpi::QuicksortBatcherMerge>(taskDataPar);
...
TEST(sarafanov_m_quick_sort_batcher_merge_mpi, task_run) {
  boost::mpi::communicator world;
  int vector_size = 524288;
  std::vector<int> data(vector_size);
  std::vector<int> result_data;
